...
import { getQueryFilters } from '../../factory/travelFactory';

@Service('TravelController')
export default class TravelController {
  constructor(@Inject('TravelService') private travelService: ITravelService) {}

  async getAllWithCompany(req: RequestWithUserData, res: Response): Promise<void> {
    try {
      const { user } = req;
      const companies = await this.travelService.getAllWithCompany(getQueryFilters(req), user);
  ...
  async getAvailableSeats(req: RequestWithUserData, res: Response): Promise<void> {
    try {
      const { user } = req;
